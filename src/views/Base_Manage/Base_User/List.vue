<template>
  <a-card :bordered="false">
    <div class="table-operator">
      ...
      <a-button v-if="hasPerm('Base_User.Delete')" type="primary" icon="minus" @click="handleDelete(selectedRowKeys)" :disabled="!hasSelected()" :loading="loading">删除</a-button>
    </div>
...
const columns = [
  ...
  { title: '管理部门', dataIndex: 'DepartmentName', width: '20%' },
...
export default {
  ...
  methods: {
    ...
    async getDeptList() {
      var re = await this.$http.get('/Base_Manage/Base_Department/GetDataList')
      this.deptList = re.Data
      console.log("deptlist", re);
      ...
      this.$http
        ...
        .then(resJson => {
          resJson.Data.forEach(p => {
            p.DepartmentName = ''
            p.DepartmentId.split(',').forEach(deptId => {
              let dn = this.deptList.find(item => {
                return item.Id == deptId
              }).Name 
              p.DepartmentName +=p.DepartmentName==''?dn:'|'+dn
            })
          });
          this.loading = false
