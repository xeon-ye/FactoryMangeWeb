    ...
    <a-spin :spinning="confirmLoading">
      <a-form-model ref="form" :model="entity" :rules="rules" v-bind="layout">
        ...
        <a-form-model-item label="部门" prop="DeartmentList">
          <a-tree-select v-model="DeptList" allowClear :treeData="DepartmentIdTreeData" placeholder="请选择部门" multiple treeDefaultExpandAll></a-tree-select>
        </a-form-model-item>
    ...
    openForm(id) {
      ...
      if (id) {
        this.$http.post('/Base_Manage/Base_User/GetTheData', { id: id }).then(resJson => {
          ...
          this.DeptList=[]
          resJson.Data.DepartmentId.split(',').forEach(p=>{
            this.DeptList.push(p)
          });
          console.log('user entity', this.entity)
    ...
    handleSubmit() {
      ...
      this.$refs['form'].validate(valid => {
        ...
        this.DeptList.forEach(p => {
          if (this.entity.DepartmentId == '') this.entity.DepartmentId = p
          else this.entity.DepartmentId = this.entity.DepartmentId + ',' +p
        })
