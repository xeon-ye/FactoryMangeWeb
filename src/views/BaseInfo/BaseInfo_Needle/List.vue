          ...
          <a-col :md="4" :sm="24">
            <a-form-item label="查询类别">
              <a-select allowClear v-model="queryParam.condition">
                <a-select-option key="NeedleName">针型</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :md="4" :sm="24">
...
const columns = [
  { title: '针型', dataIndex: 'NeedleName', width: '10%' },
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.loading = true
      this.$http
        .post('/BaseInfo/BaseInfo_Needle/GetDataList', {
      ...
      this.$confirm({
        ...
        onOk() {
          return new Promise((resolve, reject) => {
            thisObj.$http.post('/BaseInfo/BaseInfo_Needle/DeleteData', ids).then(resJson => {
              resolve()

              if (resJson.Success) {
                thisObj.$store.dispatch("GetNeedle")
